...
func TestGenericMap_Store(t *testing.T) {
	...
	val, ok := m.Load("key1")
	if !ok {
		t.Errorf("expected value for key1, but got none")
	}
	if val != "value1" {
...
func TestGenericMap_Load(t *testing.T) {
	var m Map[string, string]
	...
	_, ok = m.Load("key2")
	if ok {
		t.Errorf("expected no value for key2, but got one")
	}
}
...
func TestGenericMap_Delete(t *testing.T) {
	...
	_, ok := m.Load("key1")
	if ok {
		t.Errorf("expected no value for key1 after deletion, but found one")
	}
}
